...
class definability_interpolator : public CaDiCaL::Tracer
{
 public:
  ...
  void add_assumption_clause(uint64_t, const std::vector<int> &, const std::vector<uint64_t> &) override;
  ...
  void conclude_unsat(CaDiCaL::ConclusionType type, const std::vector<uint64_t>& clause_ids) override;
 ...
 private:
  ...
  struct clause_derivation_node {
    ...
    // Constructor
    clause_derivation_node(uint64_t clause_id, definability_interpolator& interpolator) : clause_id(clause_id), interpolator(interpolator) {}
    // Destructor
  ...
  void unmark_all();
  void create_derived_proofnode(uint64_t id);
  ...
  std::unordered_map<uint64_t, std::vector<uint64_t>> clause_id_to_antecedents;
  std::unordered_map<uint64_t, std::vector<int>> clause_id_to_clause;
  std::unordered_map<uint64_t, std::shared_ptr<binary_proofnode>> clause_id_to_proofnode;

  std::vector<uint64_t> delete_ids;
  std::unordered_map<uint64_t, std::shared_ptr<clause_derivation_node>> clause_id_to_derivation_node;
